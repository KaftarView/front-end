...
const HomePage = () => {
  ...
  const showSlidesSpecial = (n: number): void => {
    ...
    // Ensure slideIndex is within valid range
    if (n > slides.length) {
      setSlideIndexSpecial(1);
    }
    if (n < 1) {
      setSlideIndexSpecial(slides.length);
    }
    ...
    async function getPod() {
      try {
        const response = await axios.get(
          "https://api.cesaiust.ir/v1/public/podcasts",
          {
            headers: {
              "ngrok-skip-browser-warning": "69420",
              "Content-Type": "application/json", // Example header
  ...
  useEffect(() => {
    async function getMag() {
      try {
        const response = await axios.get(
          "https://api.cesaiust.ir/v1/public/journals",
          {
            headers: {
              "ngrok-skip-browser-warning": "69420",
              "Content-Type": "application/json", // Example header
  ...
  return (
    <div className="home-page">
      ...
      <div className="about-anjoman">
        <h1 className="about-anjoman-titel">درباره CESA</h1>
        <br />
        <p>
          انجمن علمی مهندسی کامپیوتر با هدف ارتقاء دانش فنی و علمی دانشجویان در
          زمینه‌های مختلف علوم کامپیوتر و فناوری اطلاعات تأسیس شده است. این
          انجمن با برگزاری کارگاه‌ها، سمینارها، مسابقات علمی و همکاری‌های
          بین‌المللی، فرصتی برای اعضا فراهم می‌آورد تا به روزترین مباحث و
          تکنولوژی‌ها را بشناسند و در پروژه‌های کاربردی مشارکت کنند. هدف نهایی
          انجمن، پرورش تخصص‌های نوآور و کارآفرین در حوزه‌های مهندسی کامپیوتر
          است.
        </p>
      </div>
        ...
        {newsList.map((news, index) => (
          ...
          >
            ...
            <div className="news-carousel-content">
              ...
              <p className="news-carousel-summary">
                {news.description.split(" ").slice(0, 50).join(" ") ||
                  "asdfghjklwertyuiopzxcvbnmdfghjk"}
                ...
              </p>
              <button
                onClick={() => {
                ...
                className="btn news-carousel-button"
              >
                رفتن به خبر
              </button>
            </div>
                ...
                <div className="box" key={index}>
                  <img
                    src={
                      event.banner ||
                      "https://aglowiditsolutions.com/wp-content/uploads/2022/12/Laravel-Best-Practices.png"
                    } // Assuming 'image' is a field in event
                  ...
                  <span className="product-status">
                    {event.venue_type || "در حال برگزاری"}
                  </span>{" "}
                  {/* Assuming 'status' is a field */}
                  <span className="product-price">
                    {event.basePrice + "تومان" || "قیمت ندارد"}
                  </span>
                  <button
                    onClick={() => {
      ...
      <div className="content">
        <div className="pudcast-section">
          <div className="about-anjoman-pudcast">
            <h1 className="about-anjoman-pudcast-titel">CESA Podcast</h1>
            <br />
            <p>
              پادکست اختصاصی انجمن علمی مهندسی کامپیوتر فرصتی منحصر به فرد برای
              اعضای انجمن و علاقه‌مندان به دنیای فناوری و مهندسی کامپیوتر است تا
              از جدیدترین موضوعات علمی، تکنولوژیک و روندهای نوین این حوزه‌ها
              مطلع شوند. این پادکست با هدف تبادل دانش و ارتقاء سطح علمی جامعه
              دانشگاهی، به بررسی مباحث مختلفی از جمله توسعه نرم‌افزار، هوش
              مصنوعی، امنیت سایبری، برنامه‌نویسی، اینترنت اشیاء (IoT) و تحولات
              در صنعت فناوری اطلاعات می‌پردازد. پادکست انجمن علمی مهندسی
              کامپیوتر به طور منظم با حضور اساتید، متخصصین برجسته و دانشجویان
              فعال در حوزه‌های مختلف کامپیوتر، مصاحبه و گفت‌وگوهایی جذاب و
              آموزنده تولید می‌کند. این پادکست به اعضا و شنوندگان این امکان را
              می‌دهد که در هر زمان و مکان به محتواهای علمی و تجربیات ارزشمند
              دسترسی داشته باشند و مهارت‌های خود را در این حوزه‌ها تقویت کنند.
              هدف اصلی این پادکست، گسترش فرهنگ یادگیری مستمر، الهام‌بخشی برای
              نوآوری و تشویق به پژوهش‌های کاربردی در زمینه‌های مختلف مهندسی
              کامپیوتر است.
            </p>
          </div>

          <div className="title ">
            <span className="title-name">جدیدترین پادکست‌ها</span>
            <button
              onClick={() => {
                navigate("/podcasts");
              }}
        ...
        <div className="magazin-section">
          <div className="about-anjoman-magazin">
            <h1 className="about-anjoman-magazin-titel">Turing Magazin</h1>
            <br />
            <p>
              شریه تخصصی "تورینگ" یکی از محصولات علمی و فرهنگی انجمن علمی مهندسی
              کامپیوتر است که با هدف ارتقاء سطح دانش و پژوهش در زمینه‌های مختلف
              علوم کامپیوتر و فناوری اطلاعات منتشر می‌شود. این نشریه به نام
              "تورینگ" (با اشاره به آلن تورینگ، پدر علم رایانه) نام‌گذاری شده
              است تا به‌نوعی یادآور جایگاه برجسته و تأثیرگذار این دانشمند بزرگ
              در شکل‌گیری دنیای دیجیتال و محاسبات مدرن باشد. نشریه تورینگ به
              صورت دوره‌ای منتشر می‌شود و مقالات علمی، پژوهشی، مصاحبه‌ها،
              تحلیل‌ها و گزارش‌هایی در زمینه‌های مختلف علوم کامپیوتر و فناوری
              ارائه می‌دهد. از جمله موضوعاتی که در این نشریه پوشش داده می‌شود
              می‌توان به الگوریتم‌ها و ساختارهای داده، هوش مصنوعی، یادگیری
              ماشین، امنیت سایبری، برنامه‌نویسی، محاسبات ابری، اینترنت اشیاء
              (IoT)، واقعیت افزوده و مجازی و سایر مباحث روز در دنیای فناوری
              اطلاعات اشاره کرد. هدف اصلی نشریه تورینگ، فراهم کردن بستری برای
              تبادل نظر و ارائه تحقیقات نوین در این حوزه‌ها و همچنین آشنایی
              مخاطبان با آخرین دستاوردهای علمی و صنعتی است. این نشریه فرصتی است
              برای دانشجویان، اساتید و پژوهشگران جوان تا یافته‌های علمی خود را
              با دیگران به اشتراک بگذارند و در محیطی علمی و پویا به بحث و تبادل
              اطلاعات بپردازند.
            </p>
          </div>
          <div className="title">
            <span className="title-name">جدیدترین شماره‌های نشریه</span>
            <button
              onClick={() => {
                navigate("/magazines");
              }}
          ...
          <div className="lists background-color-blue">
            {initialMagazine.length > 0 ? (
              initialMagazine.map((mag, index) => (
                <div className="box-small" key={index}>
                  <h2 className="product-title">{mag.name}</h2>{" "}
                  {/* Assuming 'title' is a field */}
